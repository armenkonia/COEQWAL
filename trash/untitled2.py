# -*- coding: utf-8 -*-
"""
Created on Wed Oct  2 13:45:23 2024
...
layer_name = layers[0]
du = gpd.read_file(gdb_path, layer=layer_name)
du_head = du.head()

du_selected = du.loc[du.Subregion == 'SLN01-SE-IDs']
crops = du_selected['MAIN_CROP'].unique()
...
du_selected['landiq crop type'] = du_selected.MAIN_CROP.map(mapping_dict)
...
#%%
du_selected['ppic crop type'] = du_selected['landiq crop type'].replace(crop_mapping)
merged_df = du_selected.merge(ag_regions_csv[['region', 'crop','price']], left_on=['Subregion', 'ppic crop type'], right_on=['region', 'crop'], how='left')
...
merged_df.to_file(gdb_path, layer='trial_crop_prices', driver="GPKG")
#%%
#FORGET HERE AND ON

du['landiq crop type'] = du.MAIN_CROP.map(mapping_dict)
du['ppic crop type'] = du['landiq crop type'].replace(crop_mapping)
merged_df = du.merge(ag_regions_csv[['region', 'crop','price']], left_on=['Subregion', 'ppic crop type'], right_on=['region', 'crop'], how='left')
merged_df.loc[merged_df['region'].isna(), 'price'] = 'crop price not available in this region'
merged_df.loc[merged_df['ppic crop type'].isna(), 'price'] = 'crop not classified'
merged_df_no_U_X = merged_df[~merged_df['CLASS2'].isin([' X', 'U'])]
#%%
merged_df.to_file(gdb_path, layer='trial_crop_prices_all', driver="GPKG")
merged_df_no_U_X.to_file(gdb_path, layer='trial_crop_prices_all_no_U_X', driver="GPKG")

#%%
crop_available = merged_df.MAIN_CROP.unique()
merged_df_no_U_X.plot()

#%%
ax = merged_df_no_U_X.plot(column='price', cmap='viridis', legend=True, figsize=(10, 8))
plt.show()

#%%
ag_regions = gpd.read_file(r"C:\Users\armen\Desktop\COEQWAL\Datasets\PPIC_CV_ag regions\PPIC_AgRegions_FIN.shp")
trial_crop_prices_all_no_U_X_inside_ppic_polygon = du = gpd.read_file(gdb_path, layer='trial_crop_prices_all_no_U_X_inside_ppic_polygon')

#%%

# Calculate the count of each unique value in the 'price' column
price_counts = trial_crop_prices_all_no_U_X_inside_ppic_polygon['price'].value_counts()
